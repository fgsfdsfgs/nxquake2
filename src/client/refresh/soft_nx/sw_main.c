...
R_CalcPalette (void)
{
	...
	static byte	palette[256][4], *in, *out;
	int		i, j;
...
CreateSDLWindow(int flags, int w, int h)
{
	...
	if (!window) fprintf(stderr, "SDL_CreateWindow(): %s!", SDL_GetError());

	renderer = SDL_CreateRenderer(window, -1, 0);
	if (!renderer) fprintf(stderr, "SDL_CreateRenderer(): %s!", SDL_GetError());

	surface = SDL_CreateRGBSurface(0, w, h, bpp, Rmask, Gmask, Bmask, Amask);
	if (!surface) fprintf(stderr, "SDL_CreateRGBSurface(): %s!", SDL_GetError());

	texture = SDL_CreateTexture(renderer,
				    SDL_PIXELFORMAT_ARGB8888,
				    SDL_TEXTUREACCESS_STREAMING,
				    w, h);
	if (!texture) fprintf(stderr, "SDL_CreateTexture(): %s!", SDL_GetError());
  fflush(stderr);
	return window != NULL;
#else
...
#if SDL_VERSION_ATLEAST(2, 0, 0)
	if (fullscreen == 1) {
		fs_flag = SDL_WINDOW_FULLSCREEN/*_DESKTOP*/;
